...
export function StackingClientProvider({ children }: Props) {
  ...
  return (
    <>
      <StackingClientContext.Provider value={{ client }}>
        {children}
      </StackingClientContext.Provider>
    </>
  );
}
...
export function useGetCycleDurationQuery() {
  ...
  return useQuery(["getCycleDuration", client], () =>
    client.getCycleDuration()
  );
}
...
export function useGetPoxOperationInfo() {
  const { client } = useStackingClient();
  if (!client) throw new Error("Expected client to be defined.");
  return useQuery(["getPoxOperationInfo", client], () =>
    client.getPoxOperationInfo()
  );
}
...
export function useGetAccountBalance() {
  const { client } = useStackingClient();
  if (!client) throw new Error("Expected client to be defined.");
  return useQuery(["getAccountBalance", client], () =>
...
export function useGetAccountBalanceLocked() {
  const { client } = useStackingClient();
  if (!client) throw new Error("Expected client to be defined.");
  return useQuery(["getAccountBalanceLocked", client], () =>
...
export function useGetSecondsUntilNextCycleQuery() {
  ...
  if (!client) throw new Error("Expected client to be defined.");
  return useQuery(
    ["getSecondsUntilNextCycle", client],
