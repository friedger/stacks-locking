...
type AbstractPoolingFormValues<N> =
  | PoolingFormIndefiniteValues<N>
  | PoolingFormLimitedValues<N>;
...
export const StackingDelegation: FC = () => {
  ...
  const [formValues, setFormValues] = useState<null | CompletedFormValues>(
    null
  );

  const { doContractCall } = useConnect();

  // const { poxInfo, address, nextCycleInfo } = useSelector(
  //   (state: RootState) => ({
  ...
  //   delegationType: yup
  //     .string()
  //     .typeError(`Make sure to choose a duration you'd like to pool for`),
  // });
            ...
            stackingInfoPanel={
              <StackingFormInfoPanel>
                <PoolingInfoCard
                  ...
                  durationInCycles={
                    values.delegationType === "limited" ? values.cycles : null
                  }
