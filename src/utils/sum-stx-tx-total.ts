...
export function sumStxTxTotal(
  ...
  if (tx.tx_type === "token_transfer") {
    return new BigNumber(tx.token_transfer.amount).plus(
      dir === "sent" ? tx.fee_rate : 0
    );
  }
  if (
    tx.tx_type === "coinbase" ||
  ...
  const sumEventTransferHandler = (
    prev: BigNumber,
    current: TransactionEvent
  ) =>
    current.event_type === "stx_asset" &&
    current.asset.asset_event_type === "transfer"
      ? new BigNumber(current.asset.amount || 0).plus(prev)
      : initialValue;
  return tx.events
    .reduce(sumEventTransferHandler, initialValue)
    .plus(tx.fee_rate);
}
